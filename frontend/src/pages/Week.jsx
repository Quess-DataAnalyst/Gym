...
import { Dumbbell, Waves, Timer as TimerIcon, Activity, Coffee, Moon } from "lucide-react";
...
export default function Week({ tick }) {
  ...
  const stats = computeThisWeek((idx) => PLAN[idx]);
  void tick;
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {WEEK_ORDER.map((idx) => {
            ...
            return (
              ...
              >
                <span className={`font-display text-xl uppercase ${isSel ? "text-white" : "text-neutral-300"}`}>
                  {d.short}
                </span>
                <span
                ...
                />
                {isToday && !isSel && (
                  <span className="absolute -top-1 -right-1 text-[8px] uppercase tracking-widest bg-[#FF4500] text-white rounded-sm px-1 py-[1px] font-bold">
                    Now
                  </span>
                )}
              </button>
          ...
          <>
            <div className="relative h-40 rounded-2xl overflow-hidden mb-4 border border-[#1F1F1F]">
              ...
              <div
                ...
                style={{ background: "linear-gradient(180deg, rgba(5,5,5,0.25) 0%, rgba(5,5,5,0.85) 100%)" }}
              />
                ...
                <div className="space-y-2">
                  {block.exercises.map((ex) => {
                    ...
                    return (
                      <div
                        key={ex.id}
                        data-testid={`week-exercise-${ex.id}`}
                        className="flex items-center gap-3 bg-[#111111] border border-[#1F1F1F] rounded-xl px-4 py-3"
                      >
